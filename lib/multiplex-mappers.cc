    ...
    void MapSinglePanel( int x, int y, int *matrix_x, int *matrix_y ) const {
        ...
        if ( is_top_half ) {
            *matrix_x = is_left_half ? x + panel_cols_ / 2 : x + panel_cols_;
            printf( "//// ( %3d, %3d ) ////  TOP HALF   ////: matrix_x: %3d  panel_rows: %3d  panel_cols:%3d", x, y, *matrix_x, panel_rows_, panel_cols_ );
        } else {
            printf( "//// ( %3d, %3d ) //// BOTTOM HALF ////: matrix_x: %3d  panel_rows: %3d  panel_cols:%3d", x, y, *matrix_x, panel_rows_, panel_cols_ );
