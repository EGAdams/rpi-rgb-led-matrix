...
void GameWinSequence::run( Player* player, GameState* gameState, 
    ...
    if ( MATRIX_DISABLED == 1 ) { 
        std::cout << "GameWinSequence::run() MATRIX_DISABLED == 1, not returning... " << std::endl;
        for ( int blink_sequence_count = 0; blink_sequence_count < LOOP_GAME_LAMP_WIN; blink_sequence_count++ ) {
            ...
            // player->number() == PLAYER_1_INITIALIZED ? gameState->setCurrentAction( PLAYER_1_BLINK ) 
            //                     : gameState->setCurrentAction( PLAYER_2_BLINK );
