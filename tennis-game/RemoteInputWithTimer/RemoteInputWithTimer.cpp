...
RemoteInputWithTimer::RemoteInputWithTimer(Blinker* blinker, Inputs* inputs, unsigned long timeout)
    : IInputWithTimer(blinker, timeout),  // Initialize base class
      _inputs(inputs) {                   // Initialize member variables
    _time_slept = 0;                      // Optional: If needed explicitly
}
...
unsigned long RemoteInputWithTimer::getTimeSlept() { return _time_slept; }
