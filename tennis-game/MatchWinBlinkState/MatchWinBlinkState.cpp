...
void MatchWinBlinkState::handleInput( RemoteListenerContext& context ) {
    ...
    while ( context.getMatchWinBlinker()->isRunning() ) { // _running.load() set to false when blinking is complete
...
void MatchWinBlinkState::startBlinking( RemoteListenerContext& context ) {
    if ( !context.getMatchWinBlinker()->isRunning() ) { context.getMatchWinBlinker()->start(); }
}
